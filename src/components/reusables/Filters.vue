...
const filters = ref<MovieQueryParams>({
  ...
  with_keywords: props.movieFilters?.with_keywords ?? undefined,
  with_genres: props.movieFilters?.with_genres ?? undefined
})
...
const { languages, isLoading } = storeToRefs(languagesStore)
const { getLanguages } = languagesStore
...
const handleKeywordSelection = (id: number) => {
  if (id === undefined) return (filters.value.with_keywords = undefined)
...
<template>
  <div class="filters">
    ...
    <div class="filter-item">
      ...
      <div class="relative">
        <Select v-model="filters.language" :disabled="isLoading">
          <SelectTrigger class="w-[180px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectGroup v-if="!isLoading">
              <SelectItem v-for="lang in languages" :key="lang.iso_639_1" :value="lang.iso_639_1">
                {{ lang.english_name }} ({{ lang.iso_639_1 }})
              </SelectItem>
            </SelectGroup>
            <div v-else class="p-2 text-center text-sm text-muted">Loading languages...</div>
          </SelectContent>
        </Select>
        <div
          v-if="isLoading"
